...
namespace listx
{
	...
	void Object::scan()
	{
		...
		while (e != NULL)
		{
			std::cout << "Ключ: " << ((AAA*)e->data)->key << "; " << "Число: " << ((AAA*)e->data)->mas << "\t";        //Поочерёдный вывод элементов списка. Список - это отдельный элемент таблицы
	...
	void Object::scan1(void* data)
	{
		...
		while (e != NULL)
		{
			...
			{
				std::cout << "Ключ: " << ((AAA*)e->data)->key << "; " << "Число: " << ((AAA*)e->data)->mas << "\t" << std::endl;        //Поочерёдный вывод элементов списка. Список - это отдельный элемент таблицы
			}
			e = e->getNext();
		};
	}
